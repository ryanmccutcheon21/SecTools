...
def fgsm_attack(     
	model: nn.Module, x01: np.ndarray, y: int, epsilon: float ) -> np.ndarray:     
...
# Run attack and evaluate
ch = requests.get(f"{BASE_URL}/challenge", timeout=10).json()
lab = int(ch["label"]) 
epsilon = float(ch["epsilon"] * 0.95) 
x2d = x01_from_b64_png(ch["image_b64"])      # (28, 28); original image
x4d = x2d[None, None, ...]  
x01=x4d.astype(np.float32) # numeric bound

x_adv_fgsm = fgsm_attack(model, x01, lab, epsilon)

res = requests.post(f"{BASE_URL}/predict", json={"image_b64": b64_png_from_x01(x2d)}, timeout=10).json()
print({"baseline_label": lab, "server_pred": res["pred"], "epsilon": epsilon})
